...
//
// Both functions below return a result object rather than throwing, so
// the caller can tell "Blob isn't configured/reachable, fall back"
// apart from "Blob is configured but this particular read/write failed
// for another reason" without re-parsing error messages at the call
// site.
...
async function tryReadBlobStore() {
  try {
    ...
    return { ok: true, store: JSON.parse(text) };
  } catch (e) {
