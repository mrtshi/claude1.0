...
//
// Authentication: the SDK itself resolves credentials automatically
// (explicit token > OIDC > BLOB_READ_WRITE_TOKEN), we don't need to pass
// anything explicitly. We only need to detect *whether* some form of
// Blob auth is configured, so we know whether to use Blob or fall back
// to the local file for development.
//
// For local development (or any environment without Blob configured),
// we transparently fall back to a JSON file on disk so `npm run dev`
...
// Vercel now connects Blob stores to projects via OIDC by default (more
// secure — short-lived, auto-rotating tokens) instead of the older
// static BLOB_READ_WRITE_TOKEN. When connected via OIDC, Vercel injects
// VERCEL_OIDC_TOKEN and BLOB_STORE_ID into the runtime automatically
// instead of a read-write token, so we need to detect either auth path.
const hasStaticToken = Boolean(process.env.BLOB_READ_WRITE_TOKEN);
const hasOidcAuth = Boolean(process.env.VERCEL_OIDC_TOKEN && process.env.BLOB_STORE_ID);
const hasBlobToken = hasStaticToken || hasOidcAuth;
